...
HXLINE( 119)					_hx_tmp = (::Math_obj::isNaN(thumbWidth) == false);
            				}
            				else {
...
HXLINE( 133)			Float _hx_tmp = this->get_component()->get_width();
HXDLIN( 133)			Float _hx_tmp1 = (_hx_tmp - inc->get_width());
HXDLIN( 133)			inc->set_left((_hx_tmp1 - this->get_paddingRight()));
            		}
...
HXLINE( 145)				Float x = deinc->get_width();
HXDLIN( 145)				x1 = (x1 + (x + this->get_horizontalSpacing()));
            			}
...
HXLINE( 161)			Float ucx1 = inc->get_width();
HXDLIN( 161)			ucx = (ucx - (ucx1 + this->get_horizontalSpacing()));
            		}
